...
const Header = () => {
	...
	return (
		<header className="fixed z-10 flex items-center justify-between w-screen h-20 bg-slate-500">
			<Icon className="p-2 w-14 h-14 " src={phoneBook} />
			<p className="text-4xl font-semibold text-white ">My contacts</p>
			<Button className="flex items-center justify-center p-2 m-2 text-4xl bg-green-400 rounded-full w-14 h-14 ">
				<MdPersonAddAlt1 onClick={() => onAddHandler()} />
			</Button>
		</header>
	);
};
